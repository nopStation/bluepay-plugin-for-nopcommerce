...
namespace Nop.Plugin.Payments.BluePay.Controllers
{
    public class PaymentBluePayController : BasePaymentController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Rebilling(IFormCollection form)
        {
            var storeScope = await _storeContext.GetActiveStoreScopeConfigurationAsync();
            var bluePayPaymentSettings = await _settingService.LoadSettingAsync<BluePayPaymentSettings>(storeScope);
            ...
            if (!bpManager.CheckRebillStamp(form))
            {
                await _logger.ErrorAsync("BluePay recurring error: the response has been tampered with");
                return new StatusCodeResult((int)HttpStatusCode.OK);
            }

            var authId = bpManager.GetAuthorizationIdByRebillId(form["rebill_id"]);
            if (string.IsNullOrEmpty(authId))
            {
                await _logger.ErrorAsync($"BluePay recurring error: the initial transaction for rebill {form["rebill_id"]} was not found");
                return new StatusCodeResult((int)HttpStatusCode.OK);
            }

            var initialOrder = await _orderServiceCustom.GetOrderByAuthorizationTransactionIdAndPaymentMethodAsync(authId, "Payments.BluePay");
            if (initialOrder == null)
            {
                await _logger.ErrorAsync($"BluePay recurring error: the initial order with the AuthorizationTransactionId {form["rebill_id"]} was not found");
                return new StatusCodeResult((int)HttpStatusCode.OK);
            }
            ...
            if (recurringPayment != null)
            {
                switch (form["status"])
                {
                    ...
                    case "error":
                        ...
                        processPaymentResult.Errors.Add($"BluePay recurring order {initialOrder.Id} {form["status"]}");
                        await _orderProcessingService.ProcessNextRecurringPaymentAsync(recurringPayment, processPaymentResult);
                        break;
                    case "deleted":
                    case "stopped":
                        await _orderProcessingService.CancelRecurringPaymentAsync(recurringPayment);
                        await _logger.InformationAsync($"BluePay recurring order {initialOrder.Id} was {form["status"]}");
                        break;
                }
            }
